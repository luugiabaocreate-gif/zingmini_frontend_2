...
function openCommentBox(postId) {
  ...
  // 🔹 Thêm đoạn này để tải các comment cũ từ backend
  (async () => {
    try {
      ...
      if (Array.isArray(comments) && comments.length) {
        comments.forEach((c) => {
          ...
          item.innerHTML = `<b>${escapeHtml(c.userName || "Ẩn danh")}:</b> ${escapeHtml(c.text)}`;
          list.appendChild(item);
        });
      }
    } catch (err) {
      console.warn("Không tải được comment:", err);
    }
  })();

  // 🔹 Gửi bình luận mới và lưu vào backend
  sendBtn.addEventListener("click", async () => {
    const text = input.value.trim();
    if (!text) return;

    // Hiển thị bình luận tạm ngay lập tức
    const item = document.createElement("div");
    item.style.padding = "4px 6px";
    item.style.background = "#f6fbff";
    item.style.borderRadius = "6px";
    item.innerHTML = `<b>${escapeHtml(currentUser.name || "Bạn")}:</b> ${escapeHtml(text)}`;
    list.appendChild(item);
    input.value = "";
