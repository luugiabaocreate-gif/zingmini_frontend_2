...
document.addEventListener("DOMContentLoaded", () => {
  ...
  btnPost.addEventListener("click", async () => {
    ...
    if (!text && !file) {
      alert("Hãy nhập nội dung hoặc chọn file!");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(data.message || "Đăng thất bại");
      }
    } catch (err) {
...
async function loadFeed() {
  try {
    ...
    feed.innerHTML = data
      .map((p) => {
        const text = p.text || "";
        const user = p.userName || "Người dùng";
        const fileUrl = p.fileUrl || "";
        let media = "";

        if (/\.(jpg|jpeg|png|gif)$/i.test(fileUrl)) {
          media = `<img src="${fileUrl}" alt="post" />`;
        } else if (/\.(mp4|webm)$/i.test(fileUrl)) {
          media = `<video controls src="${fileUrl}"></video>`;
        } else if (fileUrl) {
          media = `<a href="${fileUrl}" target="_blank">📎 File đính kèm</a>`;
        }
